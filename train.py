...
def train(model, config, train_iterator, dev_iterator):
    ...
    # AdamW = Adam + L2正则化
    # 输入包含两个字典：
    # param: 指定的初始化参数（将预训练模型参数作为初始化参数）
    ...
    for epoch in range(config.num_epochs):
        ...
        for _, (batch, labels) in enumerate(train_iterator):
            ...
            if total_batch % config.log_batch == 0:
                ...
                # 若当前误差小于最小误差，则记录当前误差
                # 并保存模型，记录误差修改的batch轮次：last_improve = total_batch
    ...
    #     if name == "bert.pooler.dense.weight" or name == "bert.pooler.dense.bias" or name == "classifier.weight" or name == "classifier.bias":
